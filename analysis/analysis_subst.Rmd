...
varsCont <- AskData %>% ungroup() %>% select(siblings, siborder, numchild, CharityTreatf) %>% rename(Siblings = siblings, `Sibling order` = siborder, `Num. of children`= numchild) %>% as.data.frame() %>%  group_by(CharityTreatf)
