...
TableDonationExeter <- AskDataExeter %>% ungroup() %>% drop_na(Donation, Treatment) %>% group_by(Treatment, Stage) %>% dplyr::summarize(count=n(), Donation = mean(Donation, na.rm = F))
TableDonationExeter
...
AskDataExeter %>% ungroup() %>% ggplot(aes(x = as.factor(Stage), y = Donation, color = as.factor(Treatment) )) + stat_summary(fun.y=mean, geom="point", shape=18, size=3)
