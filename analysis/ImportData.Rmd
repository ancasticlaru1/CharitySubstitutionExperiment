...
AskData <-AskData %>% select(code, Stage, CharityTreatf, Charity, Donation, Happiness, Student, one_of(varsToKeep)) %>% group_by(code) %>% filter((!is.na(Donation)& Stage == 2) |Stage == 1)

AskData <- AskData %>% arrange(code, desc(Stage)) %>% fill(CharityTreatf) 

AskData %>% ungroup() %>% group_by(Charity,Stage,CharityTreatf) %>% drop_na(Donation) %>% ggplot(aes(x = as.factor(Stage), y = Donation, color = CharityTreatf )) + stat_summary(fun.y=mean, geom="point", shape=18, size=3)

AskDataNA <- AskData %>% filter(!is.na(Donation) & is.na(code))


table(AskData$CharityTreatf, AskData$Stage, useNA = "ifany")
```
