...
AskData <- AskData %>% mutate(Donation1 = ifelse(is.na(OxfamDonation), BHFDonation, OxfamDonation))
AskData <- AskData %>% mutate(Donation2 = ifelse(is.na(ask_cruk_10_mx25a), ask_stc_10_mx25, ask_cruk_10_mx25a))




AskData <- AskData %>% gather(key = "AskCharity", value = "Donation", c("firstasktreat"), na.rm = TRUE ) 

AskData <- AskData %>% mutate(Stage = case_when(AskCharity == "s1_don_int_ha" ~ "First",
                                                AskCharity == "ns1_donint_ha" ~ "First"
                                                ))

select(AskData,Stage)
select(AskData,AskCharity)
```
